...
import DeferredDashboardMiniMap from './DeferredDashboardMiniMap';
      ...
      <main className={`mx-auto flex max-w-7xl flex-col px-4 pb-24 sm:px-6 lg:px-8 lg:pb-12 ${personalizationEnabled && preferences.density === 'compact' ? 'text-[0.96rem]' : ''}`}>
        ...
        <DashboardSectionSurface id="dashboard-urgent" label="긴급·최근 공식 사건" moduleId="urgent-cases" className="mt-5 p-5 sm:p-6 lg:p-8">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {visiblePersons.slice(0, 6).map((person) => {
              ...
              return (
                ...
                >
                  {(impressionRef) => (
                    <article ref={impressionRef} className="rounded-xl border border-slate-200 bg-white p-3 transition hover:-translate-y-0.5 hover:shadow-md">
                      <button type="button" onClick={() => selectMiniMapPerson(person.id)} className="flex w-full gap-3 text-left">
                        <div className="h-24 w-20 flex-none overflow-hidden rounded-lg bg-slate-100"><PersonPhoto person={person} compact /></div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2"><span className="rounded bg-blue-50 px-1.5 py-1 text-[10px] font-black text-[#1e3a5f]">공식정보</span>{urgent && <span className="rounded bg-red-50 px-1.5 py-1 text-[10px] font-black text-[#d94841]">24H</span>}</div>
                          <h3 className="mt-2 truncate font-black text-slate-950">{person.name}</h3>
                          <p className="mt-1 text-xs text-slate-500">{person.age ? `${person.age}세` : '나이 미상'} · {genderLabel(person.gender)}</p>
                          <p className="mt-2 flex items-start gap-1 text-xs leading-5 text-slate-600"><MapPin className="mt-0.5 flex-none" size={13} /><span className="line-clamp-2">{person.location.address}</span></p>
                        </div>
                      </button>
                    </article>
                  )}
