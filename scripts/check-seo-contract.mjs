...
const sharePage = require('../functions/lib/share/sharePage.js');
...
assert.match(html, /"url":"https:\/\/missingalert\.kr\/share\/safe-182-case-1"/);
...
assert.equal(rewriteSources[0], '/share/**');
assert.equal(firebaseConfig.hosting.rewrites[0].function.functionId, 'renderSharePage');
...
assert.match(indexHtml, /missingalert-share-fallback-v1\.png/);
assertInlineScriptsParse(indexHtml, 'home');
const activeShareHtml = sharePage.buildActiveShareHtml({...person, location: {address: person.address}});
assert.match(activeShareHtml, /<meta property="og:title"/);
assert.match(activeShareHtml, /<meta property="og:image"/);
assert.match(activeShareHtml, /<meta property="og:url" content="https:\/\/missingalert\.kr\/share\/safe-182-case-1"/);
assert.match(activeShareHtml, /<link rel="canonical" href="https:\/\/missingalert\.kr\/missing\/safe-182-case-1"/);
assert.doesNotMatch(sharePage.buildActiveShareHtml({...person, name: '<script>alert(1)</script>', photo: 'javascript:alert(1)'}), /<script>alert\(1\)<\/script>/);
assert.match(sharePage.buildUnavailableShareHtml(), /noindex,nofollow,noarchive/);
